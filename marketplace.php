...
<td><span class="badge <?= $my['status']==='active' ? 'badge-success' : 'badge-warning' ?>"><?= ucfirst(htmlspecialchars($my['status'])); ?></span></td>
<td>
...
function showPopup(msg){
    popupMsg.innerHTML=msg+'<br><br><button class="btn btn-primary" onclick="closePopup()">OK</button>';
...
function undoDelete(id,row,btn){
    ...
    .then(res=>res.json()).then(data=>{
        if(data.success){
            ...
            row.querySelector("td:nth-child(5)").innerHTML='<span class="badge badge-success">active</span>';
...
function confirmDelete(id,row,btn){
    popupMsg.innerHTML=`🗑️ <?= t("confirm_delete") ?><br><br><button class="btn btn-danger" id="confirm-delete"><?= t("yes") ?></button><button class="btn btn-secondary" onclick="closePopup()"><?= t("no") ?></button>`;
    popup.style.display="flex";
    document.getElementById("confirm-delete").onclick=()=>{
        ...
        .then(res=>res.json()).then(data=>{
            if(data.success){
                ...
                row.querySelector("td:nth-child(5)").innerHTML='<span class="badge badge-warning">deleted</span>';
